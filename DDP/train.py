...
def train(model, train_loader, criterion, optimizer, rank, epoch, args, train_len):
    ...
    tq.set_description(f'Train Epoch [{epoch}/{args.epochs}]')
    for idx, (data, label) in enumerate(train_loader):
        if args.pin_memory: # 고정된 메모리
            ...
            data, label = data.to(rank), label.to(rank) # process Id만 지정
