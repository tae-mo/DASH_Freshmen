...
def main(save_every: int, total_epochs: int, batch_size: int, lr: float, model_depth: int, model:str,
    ...
    trainer = Trainer(model_depth=model_depth, model_type=model, train_data=train_data, test_data=test_data, save_every=save_every, lr=lr, snapshot_file=snapshot_path, proj_name=proj_name)
...
if __name__ == "__main__":
    ...
    main(save_every=args.save_every, total_epochs=args.total_epochs, batch_size=args.batch_size, lr=args.lr,
         model_depth=args.depth, model=args.model, scale=args.scale, proj_name=args.name, )
    wandb.finish()
